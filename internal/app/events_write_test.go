...
func TestExecuteEventCreateDryRunReturnsExactNormalizedRequestAndDefaultPoster(t *testing.T) {
	...
	if event := envelope.Data.Request.Event; event["status"] != "UNSAVED" || event["visibility"] != "private" || event["rsvpButtonGlyphType"] != "emojis" {
		t.Fatalf("request event = %#v, want private visibility from the default create input", event)
	}
