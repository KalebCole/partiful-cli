...
const SCHEMAS: Record<string, CommandSchema> = {
  ...
  'events.create': {
    ...
    parameters: {
      ...
      '--rsvp-deadline': { type: 'string', required: false, description: 'Close RSVPs at this date/time' },
  ...
  'events.update': {
    ...
    parameters: {
      ...
      '--rsvp-deadline': { type: 'string', required: false, description: 'Close RSVPs at this date/time' },
      '--timezone': { type: 'string', required: false, default: 'America/Los_Angeles', description: 'Timezone for natural-language dates' },
      '--poster': { type: 'string', required: false, description: 'Built-in poster ID' },
      '--poster-search': { type: 'string', required: false, description: 'Search poster library, use best match' },
      '--image': { type: 'string', required: false, description: 'Custom image file path or URL to upload' },
      '--cohost': { type: 'string[]', required: false, description: 'Co-host names (resolved from contacts)' },
  ...
  'events.clone': {
    command: 'events clone <eventId>',
    parameters: {
      eventId: { type: 'string', required: true, positional: true, description: 'Source event ID' },
      '--date': { type: 'string', required: false, description: 'New event date; overrides --shift' },
      '--shift': { type: 'integer', required: false, default: 7, description: 'Shift source date by N days' },
      '--end-date': { type: 'string', required: false, description: 'End date/time; overrides duration preservation' },
      '--title': { type: 'string', required: false, description: 'Override title' },
      '--location': { type: 'string', required: false, description: 'Override venue name' },
      '--address': { type: 'string', required: false, description: 'Override street address' },
      '--description': { type: 'string', required: false, description: 'Override event description' },
      '--capacity': { type: 'integer', required: false, description: 'Override guest limit' },
      '--private': { type: 'boolean', required: false, description: 'Make event private' },
      '--timezone': { type: 'string', required: false, description: 'Override timezone' },
      '--theme': { type: 'string', required: false, description: 'Override color theme' },
      '--effect': { type: 'string', required: false, description: 'Override visual effect' },
      '--poster': { type: 'string', required: false, description: 'Override with built-in poster ID' },
      '--poster-search': { type: 'string', required: false, description: 'Override with poster search' },
      '--image': { type: 'string', required: false, description: 'Override with custom image' },
      '--link': { type: 'string[]', required: false, description: 'Override links' },
      '--link-text': { type: 'string[]', required: false, description: 'Display text for links' },
      '--cohost': { type: 'string[]', required: false, description: 'Co-host names' },
    },
  },
  'events.cancel': {
