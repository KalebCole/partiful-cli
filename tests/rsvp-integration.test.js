...
describe('events rsvp set / explore rsvp set', () => {
  it('events rsvp set --dry-run builds an addGuest payload (GOING default)', () => {
    const out = run(['events', 'rsvp', 'set', 'EV123', '--name', 'Kaleb Cole', '--dry-run', '--yes']);
  ...
  it('--status maybe maps to the MAYBE wire enum', () => {
    const out = run(['events', 'rsvp', 'set', 'EV123', '--name', 'Kaleb', '--status', 'maybe', '--dry-run', '--yes']);
    expect(out.data.payload.data.params.rsvp.status).toBe('MAYBE');
  });

  it('--plus-one is repeatable and bumps the count', () => {
    const out = run([
      'events', 'rsvp', 'set', 'EV123', '--name', 'Kaleb',
  ...
  it('rejects an invalid --status with exit code 3', () => {
    const { stdout, exitCode } = runRaw(['events', 'rsvp', 'set', 'EV123', '--name', 'Kaleb', '--status', 'interested', '--dry-run', '--yes']);
  ...
  it('explore rsvp set forwards to the SAME handler with an identical payload', () => {
    const viaEvents = run(['events', 'rsvp', 'set', 'EV123', '--name', 'Kaleb', '--status', 'going', '--dry-run', '--yes']);
    const viaExplore = run(['explore', 'rsvp', 'set', 'EV123', '--name', 'Kaleb', '--status', 'going', '--dry-run', '--yes']);
  ...
  it('requires a name when none can be resolved (no auth profile in test)', () => {
    const { stdout } = runRaw(['events', 'rsvp', 'set', 'EV123', '--dry-run', '--yes']);
    const out = JSON.parse(stdout.trim());
    expect(out.status).toBe('error');
    expect(out.error.code).toBe(3);
